...
- 版本:0.2 · 日期:**2026-09-17**(收尾时更新) · 图例:✔ 完成 · ▶ 进行中 · ⬜ 待办 · ⏸ 暂缓
- 依据:`CLAUDE.md`(架构与修复记录) + git log
- **本文件是给新会话的接续锚点** —— 换会话、换机器,从这里读起。

## 当前指针(每次 commit 前更新)

### 🧭 新会话先看这 4 行(2026-09-17 收尾时写)

- ✅ **计划已全部完成** —— `基线 → 修 bug(4/4) → ⑤ 归档 → ⑥ 切模块(5/5) → ⑦ M6` **全部走完**。
  ⇒ **当前没有进行中的主线**;下面那份「重构计划」是**已执行完的档案**,不是待办。
- **下一步 = 文末「⏸ 挂起项」** —— 最大的一块是 **M5 的 C/D/E**(3.5–5.5 天 / ≈−1,828 行 / 高风险),
  它**卡在一个只能业务方定的前置裁决**:哪一代 Agent 是产品版本(`CODE_INVENTORY.md` **§7 第 2 条** 明说无法从代码判定)。
- 🔴 **动手前必知三条**:
  1. **`main` 已开【分支保护】** —— 必须走分支 → PR → 两个必需检查(`语法检查` / `离线测试`)全绿才合;**直推会被服务端拒绝**(已实测)。
  2. **测试一律带 `POSTGRES_DB=rag_test`** —— 否则会写真库,而那是 `agent-eval-gate` 的评测知识库(已发生过,见下方)。
  3. **环境用本仓 `venv/`**(Python 3.10.10,不含 torch 系);借来的 `ai-learning/venv` **不可用**。
- 📌 **过程记录机制**:三渠道 + 触发器,见下方「过程记录(常驻机制)」段 —— **`DEC` 必须靠 PR 那一刻的强制自问触发**,否则会断(断过一次)。
...
- **⑤ 归档(第一刀)✅ 完成(2026-09-17 · PR #13)**:删掉 `CODE_INVENTORY.md` §3-1 判定的 **16 行真·死代码**(另 1 行悬空注释 + 1 行残留空白;净 −20/+2 行)。三道门前后对比:`compileall` OK · **`OPENAPI_PATHS=59` 与 `len(app.routes)=14` 逐位未变** · pytest **37 passed / 0 failed**(改动前同环境 26 passed + 11 failed,11 条红全是 Docker 未启动的 redis 连接错,**26+11=37,一条没丢**)
  - ⚠️ **⑤ 只做了「删死代码」这一半** —— **「给被取代的代际原地加 `# STATUS: superseded → §2 组N` 标记」还没做**。原因:标记要**先判定"哪一代是产品版本"**,那正是 M5 的 C/D/E 裁决(⚠️ `CODE_INVENTORY.md` **§7「明确不确定项」第 2 条** 明说第 2 代两文件的先后**无法从代码判定**),**须业务方定,Agent 不代判**
...
- **⏸ 挂起项(新会话须知,别重复踩)**:
  ...
  - **M5 的 C/D/E 三档** ⏸ **继续挂起**(业务方 2026-09-17 裁决) ——
    ...
    **卡在一个只能业务方定的前置裁决**:哪一代 Agent 是产品版本(`CODE_INVENTORY.md` **§7 第 2 条** 明说无法从代码判定)。
